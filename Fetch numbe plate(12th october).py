...
#iltered = cv2.adaptiveThreshold(gray.astype(np.uint8), 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 17, 4)
#gray = cv2.medianBlur(gray, 3)
'''kernel = np.ones((1, 1), np.uint8)
opening = cv2.morphologyEx(filtered, cv2.MORPH_OPEN, kernel)
closing = cv2.morphologyEx(opening, cv2.MORPH_CLOSE, kernel)
or_image = cv2.bitwise_or(cv2.medianBlur(gray,5), closing)'''
...
import pytesseract
#pytesseract.tesseract.tesseract_cmd = r"C:\\Program Files\\Tesseract-OCR\\tesseract.exe"

img_pil = Image.fromarray(gray)
text = pytesseract.image_to_string(img_pil,config='--psm 11')
print(text)
##
###Image2=cv2.imread("C:\\Users\\TANMAY\\Computer vision\\car.jpg")